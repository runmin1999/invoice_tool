...
def find_repeat_name(list_excel, Repeat_name_list, out_file_name):
    ...
    if out_file_name in Repeat_name_list:
        ...
        list_excel[7] = list_excel[7].replace(".0.", ".00.").replace(
            ".0_", ".00_").replace(".0(", ".00(")
...
def re_filter(pdf_text, expression, segmentation):
    try:
        return (re_text(re.compile(expression), pdf_text)).split(segmentation, 1)[-1]
    except:
        return ""


def re_info_all(pdf_text, tax_id, invoice_name, filename):
    list_excel = []

    发票号码 = re_filter(pdf_text, r'发票号码(.*\d+)', ":")
    ...
    try:
        金额 = re_text(re.compile(r'(小写.*(.*[0-9.]+))'), pdf_text)
        if 金额 is None:
            金额 = re.findall(
    ...
    开票日期 = re_filter(pdf_text, r'开票日期(.*)', ":")

    车牌 = re_filter(
        pdf_text, r'[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁台琼使领军北南成广沈济空海]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂领学警港澳]{1}(?!\d)', ":")

    list_excel.append(invoice_name)
    list_excel.append(tax_id)
    list_excel.append(int(发票号码))
    list_excel.append("无重复")
    list_excel.append(费用类型)
    list_excel.append(float(金额))
    list_excel.append(开票日期)
    list_excel.append("")
    list_excel.append(filename)
    list_excel.append(车牌)
